...
| 平台概念 | K8s 资源 | 说明 |
|---|---|---|
| 栈 | **命名空间 = 栈名**（栈名在集群内唯一），打上 `com.vanilla.stack_id` 标签 | 命名空间不存在时自动创建（无权限则需预先创建） |
...
| 卷 | `PersistentVolumeClaim`（名 `{服务名}-{卷名}`，ReadWriteOnce） | 下架不删 PVC，与 Docker named volume 语义一致 |
